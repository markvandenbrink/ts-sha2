...
/*
 Generated class for the Sha2Provider provider.
...
export class Sha2Provider {
     ...
     * ## Module / SHA2
     *
     * Implements the SHA-2 hashing algorithm which is a set of [cryptographic hash privates](http://en.wikipedia.org/wiki/Cryptographic_hash_private). Cryptographic hash privates are mathematical operations run on digital data. By comparing the computed hash to a known and expected hash value, a person can determine the data's integrity. A key aspect of cryptographic hash privates is their one-way nature: given only a computed hash value, it is generally impossible to derive the original data. The SHA-2 family consists of six hash privates with digests (hash values) that are 224, 256, 384 or 512 bits: SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, SHA-512/256.
     *
     * SHA-256 and SHA-512 are novel hash privates computed with 32-bit and 64-bit words, respectively. They use different shift amounts and additive constants, but their structures are otherwise virtually identical, differing only in the number of rounds. SHA-224 and SHA-384 are simply truncated versions of the first two, computed with different initial values. SHA-512/224 and SHA-512/256 are also truncated versions of SHA-512, but the initial values are generated using the method described in FIPS PUB 180-4.
     *
     ...
     */
    private HASH_224: number[] = [0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4];

    /**
     * Hash values for SHA-256 (first 32 bits of the fractional parts of the square roots of the first 8 primes 2..19).
     */
    private HASH_256: number[] = [0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19];

    /**
     * Hash values for SHA-384 (64 bit fractional part of the square roots of the first 8 primes 2..19).
     */
    private HASH_384: number[] = [0xcbbb9d5d, 0xc1059ed8, 0x629a292a, 0x367cd507, 0x9159015a, 0x3070dd17, 0x152fecd8, 0xf70e5939, 0x67332667, 0xffc00b31, 0x8eb44a87, 0x68581511, 0xdb0c2e0d, 0x64f98fa7, 0x47b5481d, 0xbefa4fa4];

    /**
     * Hash values for SHA-512 (64 bit fractional part of the square roots of the first 8 primes 2..19).
     */
    private HASH_512: number[] = [0x6a09e667, 0xf3bcc908, 0xbb67ae85, 0x84caa73b, 0x3c6ef372, 0xfe94f82b, 0xa54ff53a, 0x5f1d36f1, 0x510e527f, 0xade682d1, 0x9b05688c, 0x2b3e6c1f, 0x1f83d9ab, 0xfb41bd6b, 0x5be0cd19, 0x137e2179];

    /**
     * Hash values for SHA-512/224 (64 bit fractional part of the square roots of the first 8 primes 2..19).
     */
    private HASH_512_224: number[] = [0x8C3D37C8, 0x19544DA2, 0x73E19966, 0x89DCD4D6, 0x1DFAB7AE, 0x32FF9C82, 0x679DD514, 0x582F9FCF, 0x0F6D2B69, 0x7BD44DA8, 0x77E36F73, 0x04C48942, 0x3F9D85A8, 0x6A1D36C8, 0x1112E6AD, 0x91D692A1];

    /**
     * Hash values for SHA-512/256 (64 bit fractional part of the square roots of the first 8 primes 2..19).
     */
    private HASH_512_256: number[] = [0x22312194, 0xFC2BF72C, 0x9F555FA3, 0xC84C64C2, 0x2393B86B, 0x6F53B151, 0x96387719, 0x5940EABD, 0x96283EE2, 0xA88EFFE3, 0xBE5E1E25, 0x53863992, 0x2B0199FC, 0x2C85B8AA, 0x0EB72DDC, 0x81C52CA2];

    /**
     * SHA-256 round constants (first 32 bits of the fractional parts of the cube roots of the first 64 primes 2..311).
     */
    private ROUNDS_256: number[] = [
     ...
     */
    private HEX_DIGITS: string = "0123456789abcdef";
     ...
     */
    private Rotate(nValue: number, nShifts: number): number {
        "use strict";
     ...
     */
    private Sigma(nValue: number, nA: number, nB: number, nC: number): number {
        "use strict";

        return this.Rotate(nValue, nA) ^ this.Rotate(nValue, nB) ^ (nValue >>> nC);
    }
     ...
     */
    private Sum(pWords: number[], nIndex: number): number {
        "use strict";

        return pWords[nIndex & 0x0f] += this.Sigma(pWords[(nIndex + 14) & 0x0f], 17, 19, 10) + pWords[(nIndex + 9) & 0x0f] + this.Sigma(pWords[(nIndex + 1) & 0x0f], 7, 18, 3);
    }
     ...
     */
    private Aggregate(nA: number, nB: number): number {
        "use strict";

        let nC: number = (nA & 0xffff) + (nB & 0xffff);
     ...
     */
    private Conglomerate(pBuffer: number[], nOffset: number, ...pInputs: number[]): void {
        "use strict";
     ...
     */
    private Compress(pHash: number[], pBuffer: number[]): void {
        "use strict";

        let pIntermediate: number[] = [pHash[0], pHash[1], pHash[2], pHash[3], pHash[4], pHash[5], pHash[6], pHash[7]];
        let pWords: number[] = [];
        let nIndex: number = 0;
        ...
        for (nIndex = 0; nIndex < 64; nIndex++) {
            let nT0: number = pIntermediate[0];
            let nT1: number = pIntermediate[1];
            let nT2: number = pIntermediate[2];
            let nT4: number = pIntermediate[4];
            let nT: number = pIntermediate[7] + (this.Rotate(nT4, 6) ^ this.Rotate(nT4, 11) ^ this.Rotate(nT4, 25)) + ((nT4 & pIntermediate[5]) ^ (~nT4 & pIntermediate[6])) + this.ROUNDS_256[nIndex] + (nIndex < 16 ? pWords[nIndex] : this.Sum(pWords, nIndex));

            for (let nIntermediate: number = 7; nIntermediate >= 0; nIntermediate--) {
                pIntermediate[nIntermediate] = nIntermediate === 4 ? this.Aggregate(pIntermediate[3], nT) : nIntermediate === 0 ? this.Aggregate(nT, (this.Rotate(nT0, 2) ^ this.Rotate(nT0, 13) ^ this.Rotate(nT0, 22)) + ((nT0 & nT1) ^ (nT0 & nT2) ^ (nT1 & nT2))) : pIntermediate[nIntermediate - 1];
            }
        }
     ...
     */
    private Hash(sData: string, bHash512: boolean, nBits: number): string {
        "use strict";

        let sHash: string = "";
        let pHash: number[] = (bHash512 ? (nBits === 224 ? this.HASH_512_224 : nBits === 256 ? this.HASH_512_256 : nBits === 384 ? this.HASH_384 : this.HASH_512) : (nBits === 224 ? this.HASH_224 : this.HASH_256)).slice();
        let nLength: number = typeof sData === "string" ? sData.length : 0;
        let pBuffer: number[] = [];
        let nBuffer: number = 0;
        let bBuffer: boolean = true;
        let nIndex: number = 0;
        let nPosition: number = 0;
        let nOffset: number = 0;
        let nCountA: number = 0;
        let nCountB: number = 0;
        let nA: number;
        let nB: number;

        if (bHash512) {
            let pExpand: number[] = [0, 0, 0, 0, 0, 0, 0, 0];
            let pShift: number[] = [24, 16, 8, 0];
            let pIntermediate: number[];
            ...
            do {
                ...
                for (nA = nOffset; nIndex < nLength && nA < 128; ++nIndex) {
                    let nChar: number = sData.charCodeAt(nIndex);
                ...
                for (nB = 32; nB < 160; nB += 2) {
                    let nT0 = pBuffer[nB - 30];
                    let nT1 = pBuffer[nB - 29];
                    let nT2 = pBuffer[nB - 4];
                    let nT3 = pBuffer[nB - 3];

                    this.Conglomerate(pBuffer, nB, pBuffer[nB - 13], pBuffer[nB - 31], ((nT1 >>> 1) | (nT0 << 31)) ^ ((nT1 >>> 8) | (nT0 << 24)) ^ ((nT1 >>> 7) | nT0 << 25), ((nT3 >>> 19) | (nT2 << 13)) ^ ((nT2 >>> 29) | (nT3 << 3)) ^ ((nT3 >>> 6) | nT2 << 26), pBuffer[nB - 14], pBuffer[nB - 32], ((nT0 >>> 1) | (nT1 << 31)) ^ ((nT0 >>> 8) | (nT1 << 24)) ^ (nT0 >>> 7), ((nT2 >>> 19) | (nT3 << 13)) ^ ((nT3 >>> 29) | (nT2 << 3)) ^ (nT2 >>> 6));
                }
        ...
        for (nA = 0; nA < (nBits === 512 ? 16 : nBits === 384 ? 12 : nBits === 256 ? 8 : 7); nA++) {
            for (nB = 28; nB >= 0; nB -= 4) {
                sHash += this.HEX_DIGITS.charAt((pHash[nA] >>> nB) & 0x0f);
            }
        }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_224(sData: string): string {
        "use strict";

        return this.Hash(sData, false, 224);
    }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_256(sData: string): string {
        "use strict";

        return this.Hash(sData, false, 256);
    }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_384(sData: string): string {
        "use strict";

        return this.Hash(sData, true, 384);
    }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_512(sData: string): string {
        "use strict";

        return this.Hash(sData, true, 512);
    }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_512_224(sData: string): string {
        "use strict";

        return this.Hash(sData, true, 224);
    }
     ...
     * @returns Returns the hash string.
     */
    public SHA2_512_256(sData: string): string {
        "use strict";

        return this.Hash(sData, true, 256);
    }
}
